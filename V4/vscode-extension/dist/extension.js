...
const authService_1 = __webpack_require__(82);
const promptsProvider_1 = __webpack_require__(83);
const templatesProvider_1 = __webpack_require__(84);
const commandHandlers_1 = __webpack_require__(85);
function activate(context) {
    console.log('Copilot Prompter extension is now active!');
    // Initialize services
    const apiService = new apiService_1.ApiService();
    const authService = new authService_1.AuthService(context, apiService);
    // Initialize providers
    const promptsProvider = new promptsProvider_1.PromptsProvider(apiService);
    const templatesProvider = new templatesProvider_1.TemplatesProvider(apiService);
    ...
    const chatParticipant = new chatParticipant_1.CopilotPrompterChatParticipant(apiService, authService);
    ...
    commandHandlers_1.CommandHandlers.registerCommands(context);
    // Register main commands
    context.subscriptions.push(vscode.commands.registerCommand('copilot-prompter.openPrompts', () => {
    ...
    const autoAuth = vscode.workspace.getConfiguration('copilot-prompter').get('autoAuth');
    if (autoAuth) {
    ...
    authService.onAuthenticationChanged((authenticated) => {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.CopilotPrompterChatParticipant = void 0;
const vscode = __importStar(__webpack_require__(1));
class CopilotPrompterChatParticipant {
    apiService;
    authService;
    ...
    async handler(request, _context, stream, token) {
        try {
    ...
    async handlePromptCommand(query, stream, token) {
        if (!query) {
    ...
    async handleTemplateCommand(query, stream, token) {
        if (!query) {
    ...
    async handleSearchCommand(query, stream, token) {
        if (!query) {
    ...
    async handleBackendCommand(action, stream) {
        const currentBackend = vscode.workspace.getConfiguration('copilot-prompter').get('backend');
        if (!action) {
    ...
    async handleAuthCommand(stream) {
        try {
    ...
    async handleHelpCommand(stream) {
...
*Happy prompting! 🚀*`);
    ...
    async handleDefaultCommand(query, stream, token) {
        if (!query) {
            await this.handleHelpCommand(stream);
            return;
        }
    ...
    async searchAndDisplayPrompts(query, stream, token) {
        try {
    ...
    async searchAndDisplayTemplates(query, stream, token) {
        try {
    ...
    async displayPrompts(prompts, stream, token) {
        if (prompts.length === 0) {
    ...
    async displayTemplates(templates, stream, token) {
        if (templates.length === 0) {
    ...
    async provideFollowups(_result, _context, _token) {
        return [
            {
                prompt: '/search',
...
exports.CopilotPrompterChatParticipant = CopilotPrompterChatParticipant;
...
class ApiService {
    ...
    getBaseUrl() {
        ...
        const backend = config.get('backend', 'node');
        if (backend === 'node') {
            return config.get('nodeUrl', 'http://localhost:8181');
        }
        else {
            return config.get('javaUrl', 'http://localhost:8080');
        }
    }
    getApiUrl(endpoint) {
    ...
    async login(username, password) {
        ...
        const backend = vscode.workspace.getConfiguration('copilot-prompter').get('backend', 'node');
        if (backend === 'node') {
    ...
    // Prompts
    async getPrompts() {
        const response = await this.client.get(this.getApiUrl('prompts'));
    ...
    async getPrompt(id) {
        const response = await this.client.get(this.getApiUrl(`prompts/${id}`));
        // Handle Node.js response format
        if ('success' in response.data && response.data.success) {
            return response.data.data;
        }
    ...
    async searchPrompts(query, category, limit = 10) {
        let url = `prompts/search?q=${encodeURIComponent(query)}&limit=${limit}`;
        if (category) {
            url += `&category=${encodeURIComponent(category)}`;
        }
        const response = await this.client.get(this.getApiUrl(url));
        // Handle Node.js response format
        if ('success' in response.data && response.data.success) {
            return response.data.data;
        }
    ...
    async createPrompt(prompt) {
        const response = await this.client.post(this.getApiUrl('prompts'), prompt);
        // Handle Node.js response format
        if ('success' in response.data && response.data.success) {
            return response.data.data;
        }
    ...
    // Templates
    async getTemplates() {
        const response = await this.client.get(this.getApiUrl('templates'));
        // Handle Node.js response format
        if ('success' in response.data && response.data.success) {
            return response.data.data;
        }
    ...
    async getTemplate(id) {
        const response = await this.client.get(this.getApiUrl(`templates/${id}`));
        // Handle Node.js response format
        if ('success' in response.data && response.data.success) {
            return response.data.data;
        }
    ...
    async searchTemplates(query, category, language, framework, limit = 10) {
    ...
    async createTemplate(template) {
        const response = await this.client.post(this.getApiUrl('templates'), template);
        // Handle Node.js response format
        if ('success' in response.data && response.data.success) {
            return response.data.data;
        }
...
const zlib = __webpack_require__(80);
const stream = __webpack_require__(8);
const events = __webpack_require__(81);
...
var hasToStringTag = __webpack_require__(64)();
var hasOwn = __webpack_require__(63);
var $TypeError = __webpack_require__(37);
...
var $Object = __webpack_require__(31);

var $Error = __webpack_require__(32);
var $EvalError = __webpack_require__(33);
var $RangeError = __webpack_require__(34);
var $ReferenceError = __webpack_require__(35);
var $SyntaxError = __webpack_require__(36);
var $TypeError = __webpack_require__(37);
var $URIError = __webpack_require__(38);

var abs = __webpack_require__(39);
var floor = __webpack_require__(40);
var max = __webpack_require__(41);
var min = __webpack_require__(42);
var pow = __webpack_require__(43);
var round = __webpack_require__(44);
...
var $ObjectGPO = __webpack_require__(54);
var $ReflectGPO = __webpack_require__(53);
...
var reflectGetProto = __webpack_require__(53);
var originalGetProto = __webpack_require__(54);

var getDunderProto = __webpack_require__(55);
...
var $call = __webpack_require__(59);
var $actualApply = __webpack_require__(60);
...
module.exports = function () {
  if (!debug) {
    try {
      ...
      debug = __webpack_require__(70)("follow-redirects");
    }
    catch (error) { /* */ }
...
try {
	...
	const supportsColor = __webpack_require__(76);
...
/* harmony export */ });
/* harmony import */ var node_process__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(77);
/* harmony import */ var node_os__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(78);
/* harmony import */ var node_tty__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(79);
...
class AuthService {
    context;
    apiService;
    TOKEN_KEY = 'copilot-prompter.auth';
    TOKEN_EXPIRY = 24 * 60 * 60 * 1000; // 24 hours
    authChangeEmitter = new vscode.EventEmitter();
    constructor(context, apiService) {
        this.context = context;
        this.apiService = apiService;
        this.loadStoredAuth();
    }
    onAuthenticationChanged = this.authChangeEmitter.event;
    loadStoredAuth() {
        try {
            const stored = this.context.globalState.get(this.TOKEN_KEY);
    ...
    isTokenValid(auth) {
        ...
        const isSameBackend = auth.backend === vscode.workspace.getConfiguration('copilot-prompter').get('backend');
    ...
    async storeAuth(token, user) {
        const auth = {
            token,
            user,
            backend: vscode.workspace.getConfiguration('copilot-prompter').get('backend', 'node'),
            timestamp: Date.now()
        };
    ...
    async onBackendChanged() {
        ...
        if (stored && stored.backend !== currentBackend) {
            await this.clearStoredAuth();
            vscode.window.showWarningMessage('Backend changed. Please authenticate again.', 'Authenticate').then(choice => {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.PromptsProvider = exports.CategoryItem = exports.PromptItem = void 0;
const vscode = __importStar(__webpack_require__(1));
class PromptItem extends vscode.TreeItem {
    prompt;
    collapsibleState;
    constructor(prompt, collapsibleState) {
    ...
    getChildren(element) {
        if (!element) {
            ...
            if (this.groupByCategory) {
                ...
                return Promise.resolve(this.prompts.map(prompt => new PromptItem(prompt, vscode.TreeItemCollapsibleState.None)));
            }
        }
        else if (element instanceof CategoryItem) {
            // Show prompts in category
            return Promise.resolve(element.prompts.map(prompt => new PromptItem(prompt, vscode.TreeItemCollapsibleState.None)));
        }
        else {
    ...
    async loadPrompts() {
        try {
            ...
            if (error instanceof Error && error.message.includes('401')) {
                vscode.window.showWarningMessage('Authentication required to load prompts', 'Authenticate').then(choice => {
                    if (choice === 'Authenticate') {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.TemplatesProvider = exports.TemplateCategory = exports.TemplateItem = void 0;
const vscode = __importStar(__webpack_require__(1));
class TemplateItem extends vscode.TreeItem {
    template;
    collapsibleState;
    constructor(template, collapsibleState) {
    ...
    getFrameworkIcon(framework, language) {
        const iconMap = {
            'React': 'react',
            'Angular': 'angular',
            'Vue': 'vue',
...
class TemplateCategory extends vscode.TreeItem {
    category;
    templates;
    constructor(category, templates) {
        super(category, vscode.TreeItemCollapsibleState.Collapsed);
        this.category = category;
...
class TemplatesProvider {
    ...
    getChildren(element) {
        if (!element) {
            ...
            if (this.groupByCategory) {
                ...
                return Promise.resolve(this.templates.map(template => new TemplateItem(template, vscode.TreeItemCollapsibleState.None)));
            }
        }
        else if (element instanceof TemplateCategory) {
            // Show templates in category
            return Promise.resolve(element.templates.map(template => new TemplateItem(template, vscode.TreeItemCollapsibleState.None)));
        }
        else {
    ...
    async loadTemplates() {
        try {
            ...
            this.templates = [];
            if (error instanceof Error && error.message.includes('401')) {
                vscode.window.showWarningMessage('Authentication required to load templates', 'Authenticate').then(choice => {
                    if (choice === 'Authenticate') {
                        vscode.commands.executeCommand('copilot-prompter.authenticate');
                    }
                });
...
class CommandHandlers {
    static registerCommands(context) {
        // Prompt commands
        context.subscriptions.push(vscode.commands.registerCommand('copilot-prompter.copyPrompt', async (promptText, title) => {
            await vscode.env.clipboard.writeText(promptText);
            vscode.window.showInformationMessage(`Copied "${title}" to clipboard`);
        }));
        context.subscriptions.push(vscode.commands.registerCommand('copilot-prompter.insertPrompt', async (promptText) => {
        ...
        context.subscriptions.push(vscode.commands.registerCommand('copilot-prompter.openPromptDetail', async (prompt) => {
            const panel = vscode.window.createWebviewPanel('promptDetail', `Prompt: ${prompt.title}`, vscode.ViewColumn.One, {
                enableScripts: true,
        ...
        // Template commands
        context.subscriptions.push(vscode.commands.registerCommand('copilot-prompter.copyTemplate', async (templateContent, title) => {
            await vscode.env.clipboard.writeText(templateContent);
            vscode.window.showInformationMessage(`Copied "${title}" to clipboard`);
        }));
        context.subscriptions.push(vscode.commands.registerCommand('copilot-prompter.createCopilotInstructions', async (templateContent, title) => {
        ...
        context.subscriptions.push(vscode.commands.registerCommand('copilot-prompter.openTemplateDetail', async (template) => {
            const panel = vscode.window.createWebviewPanel('templateDetail', `Template: ${template.title}`, vscode.ViewColumn.One, {
                enableScripts: true,
                retainContextWhenHidden: true
            });
    ...
    static getPromptDetailHtml(prompt) {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    static getTemplateDetailHtml(template) {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
